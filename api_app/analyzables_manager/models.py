...
class Analyzable(models.Model):
    ...
    def get_all_user_events_data_model(self, user: User = None) -> BaseDataModelQuerySet:
        ...
        if user:
            query &= Q(pk__in=self.user_events.visible_for_user(user).values_list("data_model_object_id", flat=True))
        if self.classification in [
            ...
            if user:
                query2 &= Q(pk__in=self.user_domain_wildcard_events.visible_for_user(user).values_list("data_model__pk", flat=True))
            query |= query2
        elif self.classification == Classification.IP.value:
            query2 = Q(ip_wildcard_events__analyzables=self)
            if user:
                query2 &= Q(pk__in=self.user_ip_wildcard_events.visible_for_user(user).values_list("data_model__pk", flat=True))
            query |= query2
