...
class BaseAnalyzerMixin(Plugin, metaclass=ABCMeta):
    ...
    def _validate_result(self, result, level=0, max_recursion=190):
        ...
        if level == max_recursion:
            logger.info(f"We have reached max_recursion {max_recursion} level. The following object will be pruned {result} ")
            return None
...
class FileAnalyzer(BaseAnalyzerMixin, metaclass=ABCMeta):
    ...
    def filepath(self) -> str:
        ...
        if not self.__filepath:
            self.__filepath = self._job.analyzable.file.storage.retrieve(file=self._job.analyzable.file, analyzer=self.analyzer_name)
...
class DockerBasedAnalyzer(BaseAnalyzerMixin, metaclass=ABCMeta):
    ...
    def __raise_in_case_bad_request(name, resp, params_to_check=None) -> bool:
        ...
        for param in params_to_check:
            ...
            if not param_value:
                raise AnalyzerRunException(f"Unexpected Error. Please check log files under /var/log/intel_owl/{name.lower()}/")
    ...
    def _raise_container_not_running(self) -> None:
        raise AnalyzerConfigurationException(
            f"{self.name} docker container is not running.\nYou have to enable it using the appropriate parameter when executing ./start."
        )
