...
class JoeSandboxFile(FileAnalyzer, JoeSandboxMixin):
    ...
    def run(self):
        sandbox_session = JoeSandbox(apiurl=self.url, apikey=self._api_key, accept_tac=True)
        ...
        submission_id = self.create_new_analysis(sandbox_session=sandbox_session, file_details=file_details)
