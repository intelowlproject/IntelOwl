...
def migrate(apps, schema_editor):
    ...
    if not Model.objects.filter(name=dns4eu_resolver["name"]).exists():
        exists = _create_object(Model, dns4eu_resolver)
        if not exists:
    ...
    Model = apps.get_model(*python_path.split("."))
    if not Model.objects.filter(name=dns4eu_malicious_detector["name"]).exists():
