...
class DNSdb(classes.ObservableAnalyzer):
    ...
    def _get_version_endpoint(self, api_version: int):
        if api_version == 1:
            ...
            raise AnalyzerRunException(f"{api_version} not in supported versions list: {_supported_api_version}")
    ...
    def _get_health_check_url(self, user: User = None) -> Optional[str]:
        params = self._config.parameters.annotate_configured(self._config, user).annotate_value_for_user(self._config, user)
