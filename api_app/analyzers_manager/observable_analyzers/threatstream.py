        ...
        if self.threatstream_analysis == "intelligence":
            ...
            if self.observable_classification == Classification.IP:
                ...
                raise AnalyzerConfigurationException(
                    f"Observable {self.observable_classification} not supported.Currently supported are: ip, domain."
                )
        else:
            raise AnalyzerConfigurationException(
                f"Analysis type: {self.threatstream_analysis} not supported.Currently supported are: intelligence, confidence,passive_dns."
            )
        try:
