...
class InvestigationFilter(filters.FilterSet):
    ...
    def filter_for_tlp(queryset, value, tlp, *args, **kwargs):
        id_list = [investigation.id for investigation in Investigation.objects.all() if investigation.tlp == tlp]
        return queryset.filter(id__in=id_list)

    @staticmethod
    def filter_for_tags(queryset, value, tags, *args, **kwargs):
        id_list = [investigation.id for investigation in Investigation.objects.all() if tags in investigation.tags]
        return queryset.filter(id__in=id_list)
