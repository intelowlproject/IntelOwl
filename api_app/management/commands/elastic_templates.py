...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        if settings.ELASTICSEARCH_DSL_ENABLED and settings.ELASTICSEARCH_DSL_HOST:
            ...
            # push template
            with open(settings.CONFIG_ROOT / "elastic_search_mappings" / "plugin_report.json") as file_content:
                try:
                    settings.ELASTICSEARCH_DSL_CLIENT.indices.put_template(name="plugin-report", body=json.load(file_content))
