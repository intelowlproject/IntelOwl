...
def migrate(apps, schema_editor):
    ...
    for job in Job.objects.all():
        job.analyzers2_requested.set(
            AnalyzerConfig.objects.filter(name__in=job.analyzers_requested)
        )
        job.analyzers2_to_execute.set(
            AnalyzerConfig.objects.filter(name__in=job.analyzers_to_execute)
        )

        job.connectors2_requested.set(
            ConnectorConfig.objects.filter(name__in=job.connectors_requested)
        )
        job.connectors2_to_execute.set(
            ConnectorConfig.objects.filter(name__in=job.connectors_to_execute)
        )

        job.visualizers2_to_execute.set(
            VisualizerConfig.objects.filter(name__in=job.visualizers_to_execute)
        )

        job.playbooks2_requested.set(
            PlaybookConfig.objects.filter(name__in=job.playbooks_requested)
        )
        job.playbooks2_to_execute.set(
            PlaybookConfig.objects.filter(name__in=job.playbooks_to_execute)
        )
        job.full_clean()
        job.save()
...
def reverse_migrate(apps, schema_editor):
    Job = apps.get_model("api_app", "Job")
    for job in Job.objects.all():
        job.analyzers_requested = list(
            job.analyzers2_requested.all().values_list("name", flat=True)
        )
        job.analyzers_to_execute = list(
            job.analyzers2_to_execute.all().values_list("name", flat=True)
        )

        job.connectors_requested = list(
            job.connectors2_requested.all().values_list("name", flat=True)
        )
        job.connectors_to_execute = list(
            job.connectors2_to_execute.all().values_list("name", flat=True)
        )

        job.visualizers_to_execute = list(
            job.visualizers2_to_execute.all().values_list("name", flat=True)
        )

        job.playbooks_requested = list(
            job.playbooks2_requested.all().values_list("name", flat=True)
        )
        job.playbooks_to_execute = list(
            job.playbooks2_to_execute.all().values_list("name", flat=True)
        )
