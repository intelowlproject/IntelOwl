...
def migrate(apps, schema_editor):
    ...
    Job.objects.update(
        playbook_to_execute=models.Subquery(
            PlaybookConfig.objects.filter(name=models.OuterRef("old_playbook_to_execute")).values_list("pk")[:1]
        ),
        playbook_requested = models.Subquery(
            PlaybookConfig.objects.filter(name=models.OuterRef("old_playbook_requested")).values_list("pk")[:1]
    )
    )
