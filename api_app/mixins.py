...
class VirusTotalv3BaseMixin(metaclass=abc.ABCMeta):
    ...
    def _perform_request(self, uri: str, method: str, ignore_404: bool = False, **kwargs) -> Dict:
        ...
        try:
            ...
            if method == "GET":
                if kwargs:
                    logger.debug(f"about to send get request to url {url} with headers {self.headers} and kwargs: {kwargs}")
                    response = requests.get(url, headers=self.headers, **kwargs)
                else:
                    logger.debug(f"about to send get request to url {url} with headers {self.headers} and no kwargs")
                    response = requests.get(url, headers=self.headers)
            elif method == "POST":
                logger.debug(f"about to send post request to url {url} with headers {self.headers} and kwargs: {kwargs}")
    ...
    def _get_requests_params_and_uri(self, obs_clfn: str, observable_name: str, iocs: bool) -> Tuple[Dict, str, List]:
        params = {}
    ...
    def _vt_get_iocs_from_file(self, sample_hash: str) -> Dict:
        try:
            params, uri, relationships_requested = self._get_requests_params_and_uri(Classification.HASH, sample_hash, True)
            ...
            if response.status_code != 404:
                ...
                contacted_domains = [i["id"] for i in relationships.get("contacted_domains", {}).get("data", [])]
                contacted_urls = [i["context_attributes"]["url"] for i in relationships.get("contacted_urls", {}).get("data", [])]
                return {
...
class VirusTotalv3AnalyzerMixin(VirusTotalv3BaseMixin, BaseAnalyzerMixin, metaclass=abc.ABCMeta):
    ...
    def _vt_scan_file(self, md5: str, rescan_instead: bool = False) -> Dict:
        ...
        for chance in range(max_tries):
            ...
            logger.info(
                f"[POLLING] (Job: {self.job_id}, {md5}) -> GET VT/v3/_vt_scan_file #{chance + 1}/{self.max_tries} status:{analysis_status}"
            )
    ...
    def _vt_poll_for_report(
        ...
        for chance in range(self.max_tries):
            logger.info(
                f"[POLLING] (Job: {self.job_id}, observable {observable_name}) -> GET VT/v3/_vt_get_report #{chance + 1}/{self.max_tries}"
            )
    ...
    def _vt_get_report(
        ...
        params, uri, relationships_requested = self._get_requests_params_and_uri(obs_clfn, observable_name, False)
...
class JoeSandboxMixin:
    ...
    def create_new_analysis(
        ...
        if file_details:
            ...
            submission: dict = sandbox_session.submit_sample(file_details, params=params, _chunked_upload=True)
...
class RulesUtiliyMixin:
    ...
    def _check_if_latest_version(latest_version: str, python_module: PythonModule) -> bool:
        analyzer_rules_file_version = AnalyzerRulesFileVersion.objects.filter(python_module=python_module).first()
    ...
    def _download_rules(
        ...
        logger.info(f"Rules with version: {latest_version} have been successfully downloaded at {rule_set_directory}")
