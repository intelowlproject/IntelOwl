...
class PythonModule(models.Model):
    ...
    def _clean_python_module(self):
        ...
        try:
            ...
            raise ValidationError(f"`python_module` incorrect, {self.python_complete_path} couldn't be imported") from exc
...
class Job(MP_Node):
    ...
    analyzers_requested = models.ManyToManyField("analyzers_manager.AnalyzerConfig", related_name="requested_in_jobs", blank=True)
    ...
    analyzers_to_execute = models.ManyToManyField("analyzers_manager.AnalyzerConfig", related_name="executed_in_jobs", blank=True)
    ...
    def __get_single_config_reports_stats(self, config: typing.Type["AbstractConfig"]) -> typing.Dict:
        ...
        aggregators = {s.lower(): models.Count("status", filter=models.Q(status=s)) for s in AbstractReport.STATUSES.values}
    ...
    def _get_config_reports_stats(self) -> typing.Dict:
        ...
        for config in [AnalyzerConfig, ConnectorConfig, VisualizerConfig]:
            ...
            # merge them
            result = {k: result.get(k, 0) + partial_result.get(k, 0) for k in set(result) | set(partial_result)}
        return result
    ...
    def _get_signatures(self, queryset: PythonConfigQuerySet) -> Signature:
        ...
        return config_class.signature_pipeline_running(self) | group(signatures) | config_class.signature_pipeline_completed(self)
    ...
    def pivots_to_execute(self) -> PythonConfigQuerySet:
        ...
        if self.playbook_to_execute:
            ...
            pivots = PivotConfig.objects.valid(self.analyzers_to_execute.all(), self.connectors_to_execute.all())
    ...
    def get_analyzers_data_models(self) -> BaseDataModelQuerySet:
        ...
        return DataModel.objects.filter(pk__in=self.analyzerreports.values_list("data_model_object_id", flat=True))
    ...
    def get_config_runtime_configuration(self, config: "AbstractConfig") -> typing.Dict:
        try:
            ...
            raise TypeError(f"{config.__class__.__name__} {config.name} is not configured inside job {self.pk}")
    ...
    def user_month_submissions(cls, user: User) -> int:
        ...
        """
        return (
            user.job_set.filter(received_request_time__gte=timezone.now().replace(day=1, hour=0, minute=0, second=0, microsecond=0))
    ...
    def clean_scan(self):
        if self.scan_mode == ScanMode.FORCE_NEW_ANALYSIS.value and self.scan_check_time is not None:
            raise ValidationError(
                f"You can't have set mode to {ScanMode.FORCE_NEW_ANALYSIS.name} and have check_time set to {self.scan_check_time}"
            )
        elif self.scan_mode == ScanMode.CHECK_PREVIOUS_ANALYSIS.value and self.scan_check_time is None:
            raise ValidationError(f"You can't have set mode to {ScanMode.CHECK_PREVIOUS_ANALYSIS.name} and not have check_time set")
...
class PluginConfig(OwnershipAbstractModel):
    ...
    def clean_value(self):
        ...
        if type(self.value).__name__ != self.parameter.type:
            raise ValidationError(f"Type {type(self.value).__name__} is wrong: should be {self.parameter.type}")
...
class OrganizationPluginConfiguration(models.Model):
    ...
    rate_limit_timeout = models.DurationField(null=True, blank=True, help_text="Expects data in the format 'DD HH:MM:SS'")
    rate_limit_enable_task = models.ForeignKey(PeriodicTask, on_delete=models.SET_NULL, null=True, blank=True, editable=False)
...
class AbstractConfig(ListCachable):
    ...
    def get_or_create_org_configuration(self, organization: Organization) -> OrganizationPluginConfiguration:
        ...
        try:
            ...
            org_configuration = OrganizationPluginConfiguration.objects.create(config=self, organization=organization)
    ...
    def enabled_for_user(self, user: User) -> bool:
        ...
        if user.has_membership():
            return (
                not self.disabled
                and not self.orgs_configuration.filter(disabled=True, organization__pk=user.membership.organization_id).exists()
            )
...
class PythonConfig(AbstractConfig):
    ...
    def get_routing_key(self) -> str:
        ...
        if self.routing_key not in settings.CELERY_QUEUES:
            logger.warning(f"{self.name}: you have no worker for {self.routing_key}. Using {settings.DEFAULT_QUEUE} queue.")
    ...
    def get_subclasses(cls) -> typing.List["PythonConfig"]:
        ...
        return [model for model in child_models if (model is not None and issubclass(model, cls) and model is not cls)]
    ...
    def refresh_cache_keys(self, user: User = None):
        ...
        for key in cache.get_where(f"serializer_{base_key}").keys():
            ...
            cache.delete(key)
        if user:
            PythonConfigListSerializer(child=self.serializer_class()).to_representation_single_plugin(self, user)
        else:
            for generic_user in User.objects.exclude(email=""):
                PythonConfigListSerializer(child=self.serializer_class()).to_representation_single_plugin(self, generic_user)
    ...
    def signature_pipeline_running(cls, job) -> Signature:
        ...
        """
        return cls._signature_pipeline_status(job, getattr(Status, f"{cls.plugin_name.upper()}S_RUNNING").value)
    ...
    def signature_pipeline_completed(cls, job) -> Signature:
        ...
        Returns:
            Signature: The Celery task signature.
        """
        return cls._signature_pipeline_status(job, getattr(Status, f"{cls.plugin_name.upper()}S_COMPLETED").value)
    ...
    def read_configured_params(self, user: User = None, config_runtime: Dict = None) -> ParameterQuerySet:
        ...
        """
        params = self.parameters.annotate_configured(self, user).annotate_value_for_user(self, user, config_runtime)
        ...
        if not_configured_params.exists():
            ...
            if not settings.STAGE_CI or settings.STAGE_CI and not param.value:
                raise TypeError(f"Required param {param.name} of plugin {param.python_module.module} does not have a valid value")
