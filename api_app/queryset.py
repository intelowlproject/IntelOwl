...
class SendToBiQuerySet(models.QuerySet):
    ...
    def _create_index_template():
        ...
        with open(settings.CONFIG_ROOT / "elastic_search_mappings" / "intel_owl_bi.json") as f:
            ...
            settings.ELASTICSEARCH_BI_CLIENT.indices.put_template(name=settings.ELASTICSEARCH_BI_INDEX, body=body)
...
class AbstractConfigQuerySet(CleanOnCreateQuerySet):
    ...
    def alias_disabled_in_organization(self, organization):
        ...
        return self.alias(
            disabled_in_organization=Exists(opc.filter_for_config(config_class=self.model, config_pk=OuterRef("pk")).filter(disabled=True))
        )
...
class JobQuerySet(MP_NodeQuerySet, CleanOnCreateQuerySet, SendToBiQuerySet):
    ...
    def annotate_importance(self, user: User) -> QuerySet:
        ...
        Returns:
            The annotated queryset.
        """
        return self._annotate_importance_date()._annotate_importance_user(user).annotate(importance=F("date_weight") + F("user_weight"))
...
class ParameterQuerySet(CleanOnCreateQuerySet):
    ...
    def annotate_configured(self, config: "PythonConfig", user: User = None) -> "ParameterQuerySet":
        ...
        return self.annotate(
            configured=Exists(
                PluginConfig.objects.filter(parameter=OuterRef("pk"), **{config.snake_case_name: config.pk}).visible_for_user(user)
            )
        )
    ...
    def _alias_default_value(self, config: "PythonConfig") -> "ParameterQuerySet":
        ...
        return self.alias(
            default_value=Subquery(
                PluginConfig.objects.filter(parameter__pk=OuterRef("pk"), **{config.snake_case_name: config.pk})
                .default_values()
                .values("value")[:1],
            )
        )
    ...
    def _alias_runtime_config(self, runtime_config=None):
        ...
        whens = [When(name=para, then=Value(value, output_field=JSONField())) for para, value in runtime_config.items()]
    ...
    def annotate_value_for_user(self, config: "PythonConfig", user: User = None, runtime_config=None) -> "ParameterQuerySet":
        """
    ...
    def get_signatures(self, job) -> Generator[Signature, None, None]:
        ...
        for config in self:
            ...
            if not hasattr(config, "runnable"):
                raise RuntimeError("You have to call `annotate_runnable` before being able to call `get_signature`")
            # gen new task_id
