...
def run(analyzer_name, job_id, filepath, filename, md5, additional_config_params):
    ...
    try:
        ...
        result = _intezer_scan_file(
            intezer_token, md5, filename, binary, additional_config_params
        )
...
def _intezer_scan_file(intezer_token, md5, filename, binary, additional_config_params):
    ...
    if response.status_code != 201:
        raise AnalyzerRunException(
            "failed analyze request, status code {}".format(response.status_code)
        )
    ...
    for chance in range(max_tries):
        if response.status_code != 200:
            ...
            logger.info(
                "intezer md5 {} polling for result try n.{}".format(md5, chance + 1)
            )
