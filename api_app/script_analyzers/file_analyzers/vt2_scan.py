...
def run(analyzer_name, job_id, filepath, filename, md5, additional_config_params):
    ...
    try:
        ...
        wait_for_scan_anyway = additional_config_params.get('wait_for_scan_anyway', False)
        ...
        if not notify_url or wait_for_scan_anyway:
            ...
            for chance in range(max_tries):
                ...
                logger.info("vt polling, try n.{}. job_id {}".format(chance+1, job_id))
                ...
                if response_code == -2:
                    ...
                    got_result = True
                    logger.info("vt polling retrieved the result correctly for job_id {}".format(job_id))
                    break
            if not got_result:
                logger.info("max VT polls tried without getting any result. job_id {}".format(job_id))
