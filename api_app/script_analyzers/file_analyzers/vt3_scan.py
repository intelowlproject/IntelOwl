...
class VirusTotalv3ScanFile(FileAnalyzer):
    ...
    def run(self):
        ...
        return self.vt_scan_file(api_key)
    ...
    def vt_scan_file(
        ...
        if self.rescan_instead:
            logger.info(
                f"md5 {self.md5} job {self.job_id} VT analyzer requested rescan"
            )
            files = {}
            uri = f"files/{self.md5}/analyse"
        else:
            logger.info(f"md5 {self.md5} job {self.job_id} VT analyzer requested scan")
        ...
        for chance in range(self.max_tries):
            ...
            if analysis_status == "completed":
                ...
                logger.info(
                    f"vt polling: try #{chance + 1}. job_id: #{self.job_id}."
                    f" status:{analysis_status}"
                )

        if not got_result and not self.rescan_instead:
            raise AnalyzerRunException(
        ...
        return vt3_get.vt_get_report(api_key, self.md5, "hash", {}, self.job_id)
