...
def run(analyzer_name, job_id, observable_name, observable_classification, additional_config_params):
    ...
    try:
        ...
        if observable_classification == 'ip':
            ...
            raise AnalyzerRunException("not supported observable classification {}".format(observable_classification))

        result = {}
        details = otx.get_indicator_details_full(otx_type, to_analyze_observable)
        # pprint.pprint(details)

        result['pulses'] = details.get('general', {}).get('pulse_info', {}).get('pulses', [])
        result['geo'] = details.get('geo', {})
        result['malware_samples'] = [d.get('hash', '') for d in details.get('malware', {}).get('data', [])]
