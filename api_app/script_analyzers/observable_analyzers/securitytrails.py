...
def run(
    ...
    try:
        ...
        result = _securitytrails_get_report(
            api_key,
            observable_name,
            observable_classification,
            additional_config_params,
...
def _securitytrails_get_report(
    api_key, observable_name, observable_classification, additional_config_params
):
    securitytrails_analysis = additional_config_params.get(
        "securitytrails_analysis", "current"
    )
    securitytrails_current_type = additional_config_params.get(
        "securitytrails_current_type", "details"
    )
    securitytrails_history_analysis = additional_config_params.get(
    ...
    if observable_classification == "ip":
        ...
        if securitytrails_analysis == "current":
            ...
            raise AnalyzerRunException(
                "not supported analysis type {}.".format(securitytrails_analysis)
            )
    else:
        raise AnalyzerRunException(
