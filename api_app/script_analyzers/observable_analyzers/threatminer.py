...
def run(analyzer_name, job_id, observable_name, observable_classification, additional_config_params):
    ...
    try:
        result = tm_get_report(analyzer_name, observable_name, observable_classification)
...
def tm_get_report(analyzer_name, observable_name, observable_classification):

    if analyzer_name == 'Threatminer_Subdomains':
        rt_value = 5
    elif analyzer_name == 'Threatminer_PDNS':
        rt_value = 2
    elif analyzer_name == 'Threatminer_Reports_Tagging':
        rt_value = 6
    if observable_classification == 'domain':
        uri = 'domain.php'
    elif observable_classification == 'ip' and analyzer_name != 'Threatminer_Subdomains':
        uri = 'host.php'
    else:
