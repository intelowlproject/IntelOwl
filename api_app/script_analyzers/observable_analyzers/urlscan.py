...
class UrlScan(classes.ObservableAnalyzer):
    ...
    def set_config(self, additional_config_params):
        self.analysis_type = additional_config_params.get("urlscan_analysis",
                                                          "search")
    ...
    def __submit_query(self):
        data = {"url": self.observable_name, "visibility": "public"}
        uri = "api/v1/scan/"
        try:
    ...
    def run(self):
        ...
        if self.analysis_type == "search":
            params = {"q": f"domain:{self.observable_name}", "size": 100}
            uri = "api/v1/search/"
            try:
