...
def run(analyzer_name, job_id, observable_name, observable_classification, additional_config_params):
    ...
    try:
        ...
        result = vt_get_report(api_key, observable_name, observable_classification, additional_config_params,
                               job_id)
...
def vt_get_report(api_key, observable_name, observable_classification, additional_config_params,
                  job_id):
    headers = {'x-apikey': api_key}
    params = {}
    # in this way, you just retrieved metadata about relationships
    # if you like to get all the data about specific relationships, you should perform another query
    # check vt3 API docs for further info
    if observable_classification == 'domain':
        relationships_requested = ['communicating_files', 'downloaded_files', 'historical_whois', 'referrer_files',
                                   'resolutions', 'siblings', 'subdomains', 'urls']
        uri = 'domains/{}'.format(observable_name)
    elif observable_classification == 'ip':
        relationships_requested = ['communicating_files', 'downloaded_files', 'historical_whois', 'referrer_files',
                                   'resolutions', 'urls']
        uri = 'ip_addresses/{}'.format(observable_name)
    elif observable_classification == 'url':
        relationships_requested = ['downloaded_files', 'analyses', 'last_serving_ip_address',
                                   'redirecting_urls', 'submissions']
        url_id = base64.urlsafe_b64encode(observable_name.encode()).decode().strip("=")
        uri = 'urls/{}'.format(url_id)
    elif observable_classification == 'hash':
        relationships_requested = ['behaviours', 'bundled_files', 'comments', 'compressed_parents', 'contacted_domains',
                                   'contacted_ips', 'contacted_urls', 'execution_parents', 'itw_urls',
                                   'overlay_parents', 'pcap_parents', 'pe_resource_parents', 'votes']
        uri = 'files/{}'.format(observable_name)
    else:
    ...
    for chance in range(max_tries):
        try:
            logger.info("trying VT/v3 GET n.{} for job_id {}, observable {}"
                        "".format(chance+1, job_id, observable_name))
            response = requests.get(vt_base + uri, params=params, headers=headers)
            # this case is not a real error, it happens when a requested object is not found and that's normal
        ...
        if observable_classification == 'hash':
            ...
            if response.status_code == 404:
                ...
                # we should consider the chance that the very sample was already sent and...
