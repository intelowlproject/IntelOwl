...
class _AbstractJobCreateSerializer(rfs.ModelSerializer):
    ...
    investigation = rfs.PrimaryKeyRelatedField(queryset=Investigation.objects.all(), many=False, required=False, default=None)
    ...
    def validate(self, attrs: dict) -> dict:
        ...
        if attrs.get("scan_mode") == ScanMode.CHECK_PREVIOUS_ANALYSIS.value and not attrs.get("scan_check_time"):
        ...
        if not analyzers_to_execute and not connectors_to_execute:
            ...
            raise ValidationError({"detail": f"No Analyzers and Connectors can be run after filtering:\n{warnings}"})
    ...
    def set_connectors_to_execute(self, connectors_requested: List[ConnectorConfig], tlp: str, **kwargs) -> List[ConnectorConfig]:
        return list(self.plugins_to_execute(tlp, connectors_requested))

    def set_analyzers_to_execute(self, analyzers_requested: List[AnalyzerConfig], tlp: str, **kwargs) -> List[AnalyzerConfig]:
    ...
    def plugins_to_execute(
        ...
        if isinstance(plugins_requested, QuerySet):
            ...
            qs = plugins_requested[0].__class__.objects.filter(pk__in=[plugin.pk for plugin in plugins_requested])
        for plugin_config in qs.annotate_runnable(self.context["request"].user):
            try:
                if not plugin_config.runnable:
                    raise NotRunnableConnector(f"{plugin_config.name} won't run: is disabled or not configured")
                try:
...
class MultipleJobSerializer(rfs.ListSerializer):
    ...
    def validate(self, attrs: dict) -> dict:
        ...
        if len(attrs) > max_element_per_request_number:
            raise ValidationError({"detail": f"Exceed the threshold of {max_element_per_request_number}  elements for a single analysis"})
        return attrs
...
class MultipleFileJobSerializer(MultipleJobSerializer):
    ...
    def to_internal_value(self, data: QueryDict):
        ...
        if not isinstance(data, QueryDict):
            ...
            data_to_check = data
        if data_to_check.getlist("file_names", []) and len(data_to_check.getlist("file_names")) != len(data_to_check.getlist("files")):
...
class FileJobSerializer(_AbstractJobCreateSerializer):
    ...
    def set_analyzers_to_execute(
        ...
        **kwargs,
        ...
        partially_filtered_analyzers_qs = AnalyzerConfig.objects.filter(pk__in=[config.pk for config in analyzers_to_execute])
...
class JobAvailabilitySerializer(rfs.ModelSerializer):
    ...
    def create(self, validated_data):
        ...
        last_job_for_md5 = (
            Job.objects.visible_for_user(self.context["request"].user).filter(query).only("pk").latest("received_request_time")
        )
