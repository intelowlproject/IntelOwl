...
from api_app.user_events_manager.queryset import UserDomainWildCardEventQuerySet, UserIPWildCardEventQuerySet, \
    UserEventQuerySet
...
class UserEvent(models.Model):
    ...
    # internal usage
    next_decay = models.DateTimeField(default=None, editable=False, null=True, db_index=True)
...
class UserAnalyzableEvent(UserEvent):
    ...
    decay_progression = models.IntegerField(choices=DecayProgressionEnum.choices,
                                            default=DecayProgressionEnum.LINEAR.value)
...
class UserDomainWildCardEvent(UserEvent):
    ...
    analyzables = models.ManyToManyField(Analyzable, related_name="user_domain_wildcard_events", editable=False, limit_choices_to={
        "classification__in": [Classification.DOMAIN.value, Classification.URL.value]
    })

    data_model = models.ForeignKey(
        DomainDataModel, on_delete=models.CASCADE, related_name="domain_wildcard_events", editable=False
    )

    objects = UserDomainWildCardEventQuerySet.as_manager()

    def find_new_analyzables_from_query(self) -> AnalyzableQuerySet:
        return Analyzable.objects.filter(name__iregex=self.query, classification__in=[Classification.URL.value, Classification.DOMAIN.value]).exclude(pk__in=self.analyzables.values_list("pk", flat=True))
...
class UserIPWildCardEvent(UserEvent):
    ...
    analyzables = models.ManyToManyField(Analyzable, related_name="user_ip_wildcard_events", editable=False, limit_choices_to={
        "classification": Classification.IP.value
    })
    data_model = models.ForeignKey(
        IPDataModel, on_delete=models.CASCADE, related_name="domain_wildcard_events", editable=False
    )
    ...
    def find_new_analyzables_from_query(self) -> AnalyzableQuerySet:
        return Analyzable.objects.filter(classification=Classification.IP.value).annotate(ip=Cast("name", GenericIPAddressField())).filter(ip__gte=self.start_ip, ip__lte=self.end_ip).exclude(pk__in=self.analyzables.values_list("pk", flat=True))
