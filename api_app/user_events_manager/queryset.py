...
import regex
from django.db.models import QuerySet, OuterRef, Exists, Value, When, Case, F, GenericIPAddressField, Q
...
class UserEventQuerySet(QuerySet):
    ...
    def decay(self):
        ...
        objects =  self.exclude(decay_progression=DecayProgressionEnum.FIXED.value).exclude(next_decay__isnull=True).filter(
            next_decay__lte=now(),
        )
        ...
        for obj in objects:
            ...
            if obj.data_model.reliability == 0:
                ...
                if obj.decay_progression == DecayProgressionEnum.LINEAR.value:
                    ...
                    obj.next_decay += datetime.timedelta(days=obj.decay_timedelta_days ** (obj.decay_times+1))
            obj.data_model.save()
            obj.save()
...
class UserDomainWildCardEventQuerySet(UserEventQuerySet):
    ...
    def matches(self, analyzable:Analyzable) -> "UserDomainWildCardEventQuerySet":
        if analyzable.classification in [Classification.DOMAIN.value, Classification.URL.value]:
