...
class JobViewSet(ReadAndDeleteOnlyViewSet, SerializerActionMixin):
    ...
    def recent_scans_user(self, request):
        ...
        jobs = jobs.annotate_importance(request.user).order_by("-importance", "-finished_analysis_time")[:limit]
        return Response(JobRecentScanSerializer(jobs, many=True).data, status=status.HTTP_200_OK)
    ...
    def aggregate_file_mimetype(self, request):
        ...
        """
        return self.__aggregation_response_dynamic("analyzable__mimetype", users=self.get_org_members(request))
    ...
    def aggregate_top_playbook(self, request):
        ...
        """
        return self.__aggregation_response_dynamic("playbook_to_execute__name", users=self.get_org_members(request))
    ...
    def __aggregation_response_static(self, annotations: dict, users=None) -> Response:
        ...
        qs = Job.objects.filter(**filter_kwargs).annotate(date=Trunc("received_request_time", basis)).values("date").annotate(**annotations)
        return Response(qs)
    ...
    def __aggregation_response_dynamic(
        ...
        if len(most_frequent_values):
            annotations = {
                val.replace(" ", "").replace("?", "").replace(";", ""): Count(field_name, filter=Q(**{field_name: val}))
    ...
    def plugin_config(self, request, name=None):
        ...
        try:
            ...
            for attribute, param_obj in pp.data.items():
                ...
                for config in [config for config in pc.data if config["owner"] is None and config["attribute"] == attribute]:
                ...
                if request.user.has_membership():
                    ...
                    for config in [config for config in pc.data if config["organization"] == org and config["attribute"] == attribute]:
                        param_obj.update(config)
                        param_obj["exist"] = True
                ...
                for config in [
                    ...
                    if (config["owner"] == request.user.username or (request.user.is_superuser and config["ingestor_config"]))
