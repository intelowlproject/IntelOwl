...
class VisualizableTitle(VisualizableObject):
    def __init__(
        ...
        if self.disable != self.title.disable or self.disable != self.value.disable:
            logger.warning(f"Each part of the title should be disabled. Forcing all to disable={self.disable}")
...
class VisualizableVerticalList(VisualizableListMixin, VisualizableObject):
    ...
    def more_elements_object(self) -> VisualizableBase:
        ...
        if self.report:
            ...
            description = f"Inspect {self.report.config.name} {self.report.config.plugin_name.lower()} to view all the results."
            disable = False
...
class VisualizableTable(VisualizableObject):
    ...
    def to_dict(self) -> Dict:
        ...
        if any(x for x in data):
            new_data = []
            for element in data:
                new_data.append({key: value.to_dict() for [key, value] in element.items() if value is not None})
