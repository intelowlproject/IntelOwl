...
class IPReputationServices(Visualizer):
    ...
    def _vt3(self):
        try:
            ...
            hits = analyzer_report.report.get("data", {}).get("attributes", {}).get("last_analysis_stats", {}).get("malicious", 0)
    ...
    def _urlhaus(self):
        try:
            ...
            urlhaus_report = self.Title(
                ...
                self.Base(value=("" if disabled else f"found {analyzer_report.report.get('urlhaus_status', '')}")),
                disable=disabled,
            )
