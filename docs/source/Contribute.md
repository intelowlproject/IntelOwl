...
Three modules for pivots are already implemented:
1. `Compare`: this module checks if the job has exactly one related report. If the report is successful, it extracts the value to pivot from the report based on the `field_to_compare` parameter.
2. `AnyCompare`: this module inherits the `Compare` module. It checks if the job has at least one successful related report and it extracts the value to pivot from the report based on the `field_to_compare` parameter.
3. `SelfAnalyzable`: this module checks if all the job's related reports are successful and it checks that the execution of the pivot will not create an infinite loop by verifing that the playbook that we will run does not have all the analyzers that are required to run the pivot again.
