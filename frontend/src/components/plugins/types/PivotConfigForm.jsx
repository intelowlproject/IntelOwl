...
export function PivotConfigForm({ pivotConfig, toggle, isEditing }) {
  ...
  const formik = useFormik({
    initialValues: {
      ...
      python_module: pivotConfig?.python_module || "",
      soft_time_limit: pivotConfig?.soft_time_limit || 60,
      playbook: pivotConfig?.playbook_to_execute || "",
      field_to_compare: "",
    },
    validate: (values) => {
    ...
    onSubmit: async () => {
      ...
      const payloadData = {
        ...
        python_module: formik.values.python_module,
        soft_time_limit: formik.values.soft_time_limit,
        playbook_to_execute: formik.values.playbook,
        runtime_configuration: {
          pivots: {
            name: { field_to_compare: formik.values.field_to_compare },
          },
        },
      };
      ...
      if (isEditing) {
        const pivotToEdit =
          formik.initialValues.name !== formik.values.name
            ...
            : formik.values.name;
        console.debug(pivotToEdit);
        // response = await editPlaybookConfig(pivotToEdit, payloadData);
      } else {
        ...
        <div className="d-flex justify-content-start align-items-start flex-column">
          <div className="d-flex justify-content-start align-items-baseline flex-column">
            ...
            <div className="small text-left text-muted">
              ...
              Set the parameter field_to_compare with the dotted path to the
