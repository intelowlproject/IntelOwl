...
describe("test JobOverview (job report)", () => {
    ...
    test("no visualizer", async () => {
        ...
        expect(within(utilitiesRow).getByRole("button", {name: "Comments (1)"})).toBeInTheDocument();
        expect(within(utilitiesRow).getByRole("button", {name: "Delete Job"})).toBeInTheDocument();
        expect(within(utilitiesRow).getByRole("button", {name: "Rescan"})).toBeInTheDocument();
        expect(within(utilitiesRow).getByRole("button", {name: "Save As Playbook"})).toBeInTheDocument();
        expect(within(utilitiesRow).getByRole("button", {name: "Raw JSON"})).toBeInTheDocument();
        expect(within(utilitiesRow).getByRole("button", {name: "Share"})).toBeInTheDocument();
        // metadata - first line
        const JobInfoCardSection = container.querySelector('#JobInfoCardSection');
        expect(within(JobInfoCardSection).getByText("dns.google.com")).toBeInTheDocument();
        expect(within(JobInfoCardSection).getByText("Status")).toBeInTheDocument();
        expect(within(JobInfoCardSection).getByText("REPORTED WITHOUT FAILS")).toBeInTheDocument();
        ...
        expect(within(JobInfoCardSection).getByText("f9bc35a57b22f82c94dbcc420f71b903")).toBeInTheDocument();
        expect(within(JobInfoCardSection).getByText("Process Time (mm:ss)")).toBeInTheDocument();
        expect(within(JobInfoCardSection).getByText("00:00")).toBeInTheDocument();
        expect(within(JobInfoCardSection).getByText("Start Time")).toBeInTheDocument();
        expect(within(JobInfoCardSection).getByText("10:19:03 AM May 31st, 2023")).toBeInTheDocument();
        expect(within(JobInfoCardSection).getByText("End Time")).toBeInTheDocument();
        expect(within(JobInfoCardSection).getByText("10:19:04 AM May 31st, 2023")).toBeInTheDocument();
        // metadata - second line
        expect(within(JobInfoCardSection).getByText("Tags")).toBeInTheDocument();
        expect(within(JobInfoCardSection).getByText("Error(s)")).toBeInTheDocument();
        expect(within(JobInfoCardSection).getByText("Playbook")).toBeInTheDocument();
    ...
    test("visualizer running", () => {
        ...
        const goBackButton = within(utilitiesRow).getByRole("button", {name: ""})
        expect(goBackButton.id).toBe("gobackbutton")
        expect(within(utilitiesRow).getByRole("button", {name: "Comments (0)"})).toBeInTheDocument();
        expect(within(utilitiesRow).getByRole("button", {name: "Delete Job"})).toBeInTheDocument();
        expect(within(utilitiesRow).getByRole("button", {name: "Rescan"})).toBeInTheDocument();
        expect(within(utilitiesRow).getByRole("button", {name: "Save As Playbook"})).toBeInTheDocument();
        expect(within(utilitiesRow).getByRole("button", {name: "Raw JSON"})).toBeInTheDocument();
        ...
        expect(rawButton.className).toContain("btn-outline-tertiary");  // not selected
        // LOADING SPINNER
        expect(container.querySelector('#visualizerLoadingSpinner')).toBeInTheDocument();
        // raw data section not rendered
        expect(screen.queryByText("Analyzers Report")).toBeNull();
        expect(screen.queryByText("Connectors Report")).toBeNull();
        expect(screen.queryByText("Visualizers Report")).toBeNull();
    ...
    test("user interaction with visualizer section", async () => {
        ...
        */
        ...
        expect(firstVisualizerMenuElement.closest("a").className).toContain("active");
        expect(secondVisualizerMenuElement.closest("a").className).not.toContain("active");
        ...
        expect(within(firstVisualizerBody).getByText("Classic DNS (2)")).toBeInTheDocument();
        expect(within(secondVisualizerBody).getByText("test component visualizer 2")).toBeInTheDocument();
        ...
        expect(rawButton.className).toContain("btn-outline-tertiary");  // not selected
        // check nav
        expect(firstVisualizerMenuElement.closest("a").className).not.toContain("active");
        expect(secondVisualizerMenuElement.closest("a").className).toContain("active");
        // check tabs
        ...
        expect(analyzerReportMenuElement.closest("a").className).toContain("active");
        expect(connectorsReportMenuElement.closest("a").className).not.toContain("active");
        expect(visualizersReportMenuElement.closest("a").className).not.toContain("active");
        // check tabs
        ...
        expect(rawButton.className).toContain("btn-primary");  // not selected
        // check nav
        expect(analyzerReportMenuElement.closest("a").className).not.toContain("active")
        expect(connectorsReportMenuElement.closest("a").className).toContain("active")
        expect(visualizersReportMenuElement.closest("a").className).not.toContain("active")
        // check tabs
        ...
        expect(analyzerReportMenuElement.closest("a").className).not.toContain("active")
        expect(connectorsReportMenuElement.closest("a").className).not.toContain("active")
        expect(visualizersReportMenuElement.closest("a").className).toContain("active")
        // check tabs
        expect(analyzerReportBody.className).not.toContain("active")
        expect(connectorReportBody.className).not.toContain("active")
