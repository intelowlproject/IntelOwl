...
describe("TitleVisualizer component", () => {
    ...
    test("required-only params", () => {
        ...
        const {container} = render(
            <TitleVisualizer
                size="col-2"
                title={<BaseVisualizer size="auto" value="title (required-only params)" />}
                value={<BaseVisualizer size="auto" value="value (required-only params)" />}
            />
        );
        ...
        // check value
        expect(screen.getByText('value (required-only params)')).toBeInTheDocument();
