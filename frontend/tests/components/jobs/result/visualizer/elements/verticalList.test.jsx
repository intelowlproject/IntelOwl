...
describe("VerticalListVisualizer component", () => {
    ...
    test("required-only params", () => {
        ...
        const {container} = render(
            <VerticalListVisualizer
                size="col-2"
                name={<BaseVisualizer size="auto" value="title (required-only params)" color="warning" />}
                values={[
                    <BaseVisualizer size="auto" value="first line - single element" />,
                    <HorizontalListVisualizer
                        values={[
                            <BaseVisualizer size="auto" value="second line - first element" />,
                            <BaseVisualizer size="auto" value="second line - second element" />,
                        ]}
                    />
                ]}
            />
        ...
        // check title
        expect(screen.getByText('title (required-only params)')).toBeInTheDocument();
        // check values
        ...
        expect(screen.getByText('second line - second element')).toBeInTheDocument();
        // check size
    ...
    test("all params", () => {
        ...
        const {container} = render(
            <VerticalListVisualizer
                size="col-2"
                name={<BaseVisualizer size="auto" value="title (all params)" color="success" />}
                values={[
                    <BaseVisualizer size="auto" value="first line - single element" />,
                    <HorizontalListVisualizer
                        values={[
