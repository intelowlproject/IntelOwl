...
jest.mock('../../../src/stores', () => ({
    ...
    useTagsStore: jest.fn((state) => state({
        loading: false,
        error: null,
        tags: [{
            "id": 1,
    ...
    usePluginConfigurationStore: jest.fn((state) => state({
...
describe("test ScanForm component", () => {
    ...
    */
    ...
    beforeAll(() => {
        axios.post.mockImplementation(() => Promise.resolve({ data: { "results" : [],"count": 0 }}));
    })
    ...
    test("form validation - no observable selection and playbook", async () => {
        ...
        const firstObservableInputElement = screen.getByRole("textbox", { name: "" });
        ...
        const testPlaybookButton = container.querySelector(`#${  playbookDropdownButton.id.replace("-input", "")  }-option-0`);
    ...
    test("form validation - no observable and analyzer selected", async () => {
        ...
        const testAnalyzerButton = container.querySelector(`#${  analyzerDropdownButton.id.replace("-input", "")  }-option-0`);
    ...
    test("form validation - no file selection and playbook", async () => {
        ...
        expect(playbookDropdownButton).toBeInTheDocument();
        await user.click(playbookDropdownButton);
        const testAnalyzerButton = container.querySelector(`#${  playbookDropdownButton.id.replace("-input", "")  }-option-0`);
        expect(testAnalyzerButton).toBeInTheDocument();
        await user.click(testAnalyzerButton);
    ...
    test("test playbooks advanced change time", async () => {
        ...
        // advanced settings
        const advancedSettingsButton = screen.getByRole("button", {name: "Advanced settings"})
    ...
    test("test observable playbooks advanced settings (force analysis, tlp and tags)", async () => {
        ...
        const testTagButton = container.querySelector(`#${  tagsDropdown.id.replace("-input", "")  }-option-0`);
        ...
        const forceAnalysisRadio = screen.getByRole("radio", {name: "Force new analysis"})
    ...
    test("domains playbook analysis", async () => {
        ...
        const addNewValueButton = screen.getByRole("button", { name: "Add new value" });
        expect(addNewValueButton).toBeInTheDocument();
        await user.click(addNewValueButton);
        const secondObservableInputElement = screen.getAllByRole("textbox", { name: "" })[1];
    ...
    test("HASH playbook analysis", async () => {
        ...
        const firstObservableInputElement = screen.getByRole("textbox", { name: "" });
        await user.type(firstObservableInputElement, "1d5920f4b44b27a802bd77c4f0536f5a");
        // add second observable to analyze
        const addNewValueButton = screen.getByRole("button", { name: "Add new value" });
        expect(addNewValueButton).toBeInTheDocument();
        await user.click(addNewValueButton);
        const secondObservableInputElement = screen.getAllByRole("textbox", { name: "" })[1];
        // doubled braked are required by user-event library
        await user.type(secondObservableInputElement, "ff5c054c7cd6924c570f944007ccf076");
    ...
    test("file playbook analysis", async () => {
        ...
        File.prototype.text = jest.fn().mockResolvedValueOnce("this is a text line").mockResolvedValueOnce("this is another line");
        const fileInputComponent = screen.getByLabelText("File(s)");
        const testImageFiles = [
            new File(['this is a text line'], 'test1.txt', {type: 'plain/text'}),
