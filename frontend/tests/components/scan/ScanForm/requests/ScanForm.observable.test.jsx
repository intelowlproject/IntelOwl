...
import ScanForm, {sanitizeObservable} from "../../../../../src/components/scan/ScanForm";
import {
...
describe("test ScanForm component with domains", () => {
  ...
  test.each([
    ...
    ])('playbook analysis (%s)', async ({type, firstObservable, secondObservable, playbook}) => {
        ...
        if (playbook.scan_mode === parseInt(ScanModesNumeric.CHECK_PREVIOUS_ANALYSIS, 10)) {
            payload.scan_check_time = `${parseScanCheckTime(playbook.scan_check_time)}:00:00`;
        };
    ...
    ])("analyzer analysis (%s)", async ({type, firstObservable, secondObservable}) => {
      const user = userEvent.setup();
