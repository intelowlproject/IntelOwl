...
def get_secret(secret_name, default=""):
    ...
    if not secret and aws_secrets_enabled and not secret_name == "AWS_REGION":
        try:
            ...
            logging.error(f"Failed retrieving of secret {secret_name}. Error: {e}.")  # lgtm [py/clear-text-logging-sensitive-data]
        except NoCredentialsError as e:
            logging.error(f"Error: {e}. Secret: {secret_name}")  # lgtm [py/clear-text-logging-sensitive-data]
        except Exception as e:
            logging.exception(f"Error: {e}. Secret: {secret_name}")  # lgtm [py/clear-text-logging-sensitive-data]
    return secret
