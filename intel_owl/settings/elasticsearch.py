...
ELASTICSEARCH_BI_ENABLED = secrets.get_secret("ELASTICSEARCH_BI_ENABLED", False) == "True"
if ELASTICSEARCH_BI_ENABLED:
    ...
    if ELASTICSEARCH_BI_HOST and ELASTICSEARCH_BI_INDEX:
        ...
        if any("elasticsearch:9200" in host for host in ELASTICSEARCH_BI_HOST):
            ...
            elasticsearch_bi_conf["ca_certs"] = "/opt/deploy/intel_owl/certs/elastic_ca/ca.crt"
        ...
        try:
            ...
            print(f"ELASTICSEARCH BI client configuration did not connect correctly: {e}")
    else:
        print("Elasticsearch BI not correctly configured")


# advanced search
ELASTICSEARCH_DSL_ENABLED = secrets.get_secret("ELASTICSEARCH_DSL_ENABLED", False) == "True"
if ELASTICSEARCH_DSL_ENABLED:
    ...
    if ELASTICSEARCH_DSL_HOST:
        ...
        if "elasticsearch:9200" in ELASTICSEARCH_DSL_HOST:
            ...
            elastic_search_conf["ca_certs"] = "/opt/deploy/intel_owl/certs/elastic_ca/ca.crt"
        ELASTICSEARCH_DSL_CLIENT = Elasticsearch(**elastic_search_conf)
        try:
            if not ELASTICSEARCH_DSL_CLIENT.ping():
                print("ELASTICSEARCH DSL client configuration did not connect correctly")
        except Exception as e:
            print(f"ELASTICSEARCH DSL client configuration did not connect correctly: {e}")
    else:
        print("you have to configure ELASTIC_HOST with the URL of your ElasticSearch instance")
