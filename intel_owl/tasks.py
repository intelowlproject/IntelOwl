...
class FailureLoggedRequest(Request):
    ...
    def on_failure(self, exc_info, send_failed_event=True, return_ok=False):
        logger.critical(f"Failure detected for task {self.task.name} with exception {exc_info} and request {self._request_dict}")
...
def check_stuck_analysis(minutes_ago: int = 25, check_pending: bool = False):
    ...
    def fail_job(job):
        logger.error(f"found stuck analysis, job_id:{job.id}.Setting the job to status {Job.STATUSES.FAILED.value}'")
...
def send_bi_to_elastic(max_timeout: int = 60, max_objects: int = 10000):
    ...
    if settings.ELASTICSEARCH_BI_ENABLED:
        for report_class in [
            ...
            report_class.objects.filter(sent_to_bi=False).filter_completed().defer("report").order_by("-start_time")[
                :max_objects
            ].send_to_elastic_as_bi(max_timeout=max_timeout)
        Job.objects.filter(sent_to_bi=False).filter_completed().order_by("-received_request_time")[:max_objects].send_to_elastic_as_bi(
...
def send_plugin_report_to_elastic(max_timeout: int = 60, max_objects: int = 10000):
    ...
    if settings.ELASTICSEARCH_DSL_ENABLED and settings.ELASTICSEARCH_DSL_HOST:
            ...
            report_document_list = [
                {
                    "_op_type": "index",
                    "_index": (
                        f"plugin-report-{get_environment()}-{inflection.underscore(_class.__name__).replace('_', '-')}-{now().date()}"
                    ),
                    "_source": {
        ...
        if all_report_document_list:
            logger.info(
                ", ".join(
                    [f"{document['_source']['job']['id']}-{document['_source']['config']['name']}" for document in all_report_document_list]
                )
            )
            _, errors = bulk(  # noqa
