...
class BaseAnalyzerTest(TestCase):
    ...
    def _setup_analyzer(self, config, observable_type, observable_value):
        logger.info(f"Setting up analyzer {self.analyzer_class.__name__} for {observable_type}: {observable_value}")
    ...
    def _validate_response(self, response, observable_type):
        if isinstance(response, str):
            try:
                ...
                self.fail(f"{self.__class__.__name__}: Analyzer response for {observable_type} is a string but not valid JSON")
    ...
    def test_analyzer_on_supported_observables(self):
        if self.analyzer_class is None:
            self.skipTest(f"{self.__class__.__name__}.test_analyzer_on_supported_observables skipped: analyzer_class is not set")

        configs = AnalyzerConfig.objects.filter(python_module=self.analyzer_class.python_module)

        if not configs.exists():
            self.skipTest(f"{self.__class__.__name__}: No AnalyzerConfig found for {self.analyzer_class.python_module}")
            ...
            with self.subTest(observable_type=observable_type):
                ...
                with self._apply_patches(patches):
                    ...
                    try:
                        ...
                        self.fail(f"{self.__class__.__name__}: AnalyzerRunException for {observable_type}: {e}")
                    except Exception as e:
                        logger.exception(f"Unexpected exception for {observable_type}")
                        self.fail(f"{self.__class__.__name__}: Unexpected exception for {observable_type}: {type(e).__name__}: {e}")
