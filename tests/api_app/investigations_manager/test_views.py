...
class InvestigationViewSetTestCase(APITestCase):
    ...
    def setUp(self):
        ...
        self.first_investigation = Investigation.objects.create(owner=self.first_user, name="first investigation")
        self.second_investigation = Investigation.objects.create(owner=self.second_user, name="second investigation")
        self.third_investigation = Investigation.objects.create(owner=self.second_user, name="third investigation", for_organization=True)
        self.another_investigation = Investigation.objects.create(owner=self.another_org_user, name="another investigation")
        self.an, _ = Analyzable.objects.get_or_create(name="sub.test.com", classification=Classification.DOMAIN)
    ...
    def test_remove_job(self, *args, **kwargs):
        ...
        response = self.client.post(f"{self.URL}/{self.first_investigation.pk}/remove_job", data={"job": job.pk})
        self.assertEqual(response.status_code, 403)
        job.user = self.first_user
        job.save()
        response = self.client.post(f"{self.URL}/{self.first_investigation.pk}/remove_job", data={"job": job.pk})
