...
class ReadConfiguredParamsTestCase(CustomTestCase):
    ...
    def test_returns_configured_params(self):
        ...
        # VisualizerConfig "yara.Yara" has no required unconfigured secrets
        # by default, so calling read_configured_params must succeed.
        result = self.vc.read_configured_params(self.user)
        # result should be a queryset (possibly empty, but no TypeError)
    ...
    def test_stage_ci_with_value_does_not_raise(self):
        ...
        # Provide a PluginConfig value so param.value is not empty
        ...
        try:
            ...
            with patch("api_app.models.settings") as mock_settings:
                ...
                # Should NOT raise because param has a value
                result = self.vc.read_configured_params(self.user)
                self.assertIsNotNone(result)
        finally:
            pc.delete()
