...
class JobViewSetTests(CustomViewSetTestCase):
    ...
    def setUp(self):
        ...
        with patch(
            ...
            self.analyzable3 = Analyzable.objects.create(name="test.com", classification=Classification.DOMAIN)
            ...
            self.investigation1, _ = Investigation.objects.get_or_create(name="test_investigation1", owner=self.superuser)
        ...
        self.assertEqual(
            ...
            Bool(
                filter=[
                    ...
                    Range(start_time={"gte": datetime.datetime(2024, 11, 27, 0, 0, tzinfo=ZoneInfo(key="UTC"))}),
                    Range(start_time={"lte": datetime.datetime(2024, 11, 28, 0, 0, tzinfo=ZoneInfo(key="UTC"))}),
                    Range(end_time={"gte": datetime.datetime(2024, 11, 27, 0, 0, tzinfo=ZoneInfo(key="UTC"))}),
                    Range(end_time={"lte": datetime.datetime(2024, 11, 28, 0, 0, tzinfo=ZoneInfo(key="UTC"))}),
                    Term(report="8.8.8.8"),
                ]
            ),
        )
