...
class SendElasticTestCase(CustomTestCase):
    def setUp(self):
        self.user, _ = User.objects.get_or_create(username="test_elastic_user", email="elastic@intelowl.com", password="test")
        self.organization, _ = Organization.objects.get_or_create(name="test_elastic_org")
        self.membership, _ = Membership.objects.get_or_create(user=self.user, organization=self.organization, is_owner=True)
        self.analyzable = Analyzable.objects.create(name="dns.google.com", classification=Classification.DOMAIN)
                ...
                [
                    {
                        ...
                        "_source": {
                            ...
                            "job": {"id": self.job.id},
                            "start_time": datetime.datetime(2024, 10, 29, 10, 49, tzinfo=datetime.timezone.utc),
